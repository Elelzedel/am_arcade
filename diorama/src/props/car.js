...
// Every so often a little car hisses past on the wet road. It arrives
// through the cut edge of the diorama, sliced clean by the plane that bounds
// the world, as if from nowhere; at the other end the road simply stops, and
// so, after a moment of cartoon hang-time, does the car.

export function createCar(scene, { laneZ }) {
    const clip = [new THREE.Plane(new THREE.Vector3(1, 0, 0), -PLINTH.minX)];
    ...
    const state = { x: PLINTH.minX - 3, driving: false, next: 9, speed: 4.2, honk: 0, fall: null };
    ...
    return {
        ...
        update(dt, time) {
            ...
            if (!f) {
                ...
                // front wheels past the edge: over it goes
                if (state.x > PLINTH.maxX + 0.35) {
                    state.fall = { t: 0, vy: 0, spin: 0, roll: (Math.random() - 0.5) * 1.5 };
                    this.onFall?.();
                }
                return;
            }
            // the fall: a teeter on the brink, a beat of cartoon disbelief,
            // then down it goes, nose first, tumbling slowly into the dark
            f.t += dt;
            const teeter = f.t < 0.9;
            if (teeter) {
                const k = f.t / 0.9;
                root.rotation.z = -0.28 * k + Math.sin(f.t * 14) * 0.12 * (1 - k);
                state.x += state.speed * dt * 0.08 * (1 - k);
                root.position.x = state.x;
                for (const w of wheels) w.rotation.z -= dt * 40;
                return;
            }
            f.vy -= 5.5 * dt;
            state.x += state.speed * dt * 0.45;
            root.position.x = state.x;
            root.position.y += f.vy * dt;
            f.spin += dt * 1.6;
            root.rotation.z = -0.28 - f.spin;
            root.rotation.x += f.roll * dt;
            for (const w of wheels) w.rotation.z -= dt * 30;
            if (root.position.y < -40) {
                state.driving = false;
                state.fall = null;
                state.next = 18 + Math.random() * 18;
                root.position.set(PLINTH.minX - 3, 0, root.position.z);
